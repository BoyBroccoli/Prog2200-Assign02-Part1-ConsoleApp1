...
namespace Delegates
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            FeesDiscountDelegate theDiscountDel = delegate (ref decimal theFee, bool m_isHighRisk)
            {
                if (!m_isHighRisk)
                {
                    theFee -= 10;
            ...
            do
            {
                ...
                if (!theZone.Equals("exit"))
                {
                    ...
                    if (theDest != null)
                    {
                        // ask for the price and convert the string to a decimal number Part 1
                        Console.WriteLine("What is the item price?");
                                               
                        string thePriceStr = Console.ReadLine();
                        decimal itemPrice;
                        // validation while loop here
                        while (!decimal.TryParse(thePriceStr, out itemPrice)) // if thePriceStr is not a decimal will keep looping
                        {
                            Console.WriteLine("Please enter a decimal greater than 0");
                            thePriceStr = Console.ReadLine();
                                                    
                        }
                        itemPrice = decimal.Parse(thePriceStr); // storing thePriceStr into itemPrice which is a decimal after validating
                        ...
                        theDiscountDel(ref theFee, theDest.m_isHighRisk);
